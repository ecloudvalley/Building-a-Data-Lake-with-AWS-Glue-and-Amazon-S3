...
2.6. Enter data source path **s3://aws-bigdata-blog/artifacts/glue-data-lake/data/** and click **Next**.
...
3.1. On the **service** menu, click **S3**.

3.2. Click **Create bucket** button.

3.3. In **Bucket name**, type **nytaxi-original-data-xxxx** (where xxxx is you name). For example, if your name is Annie, the bucket name will be set as **nytaxi-original-data-annie**. If the bucket name has been used, please add a number to the end of bucket name.

3.4. In **Region**, select **US East (N. Virginia)**.

3.5. Click **Next** button.

3.6. Click **Next** button.

3.7. Disable the following options.

![disable.png](images/disable.png)

3.8. Click **Next** button.

3.9. Click **Create bucket** button.


### Transform the Data from CSV to Parquet Format

4.1. In the left navigation pane, under **ETL**, click **Jobs**, and then click **Add job**.

4.2. On the Job properties, enter the following details:

* **Name: nytaxi-csv-parquet**
    
* **IAM role**: choose **AWSGlueServiceRoleDefault**

4.3. For **This job runs**, select **A proposed script generated by AWS Glue**.

4.4. Click **Next**.

4.5. Choose **data** as the data source, and click **Next**.

4.6. Choose **Create tables in your data target**.

4.7. For Datastore, select Amazon S3, and select **Parquet** as the format.

4.8. For **Target path**, type **s3://aws-glue-result-xxxx** to store data where **`xxxx`** is your name.

4.9. Verify the schema mapping, and click **Finish**.

4.10. View the job. This screen provides a complete view of the job and allows you to edit, click **Save**, and click **Run job**. This steps may be waiting around 10 minutes.
...
5.1. When the job has finished, back to **AwS Glue console**, click **Tables** on left navigation pane.

5.2. Click **Add tables ->　Add tables using a crawler**, type Crawler name **nytaxiparquet** and click **Next**.

5.3. Choose S3 as the **Data store**.

5.4. Include path type **s3://aws-glue-result-xxxx** (where **`xxxx`** is your name) to store data.

5.5. Click **Next**.

5.6. On **Add another data store** page, select **No**, and click **Next**.

5.7. Select **Choose an existing IAM role**, and select the role  **AWSGlueServiceRoleDefault** you just created in the drop-down list, and click **Next**.

5.8. For **Frequency**, choose **Run on demand**, and click **Next**.

5.9. For **Database**, choose **nycitytaxi**, and click **Next**.

5.10. Review the steps, and click **Finish**.

5.11. The crawler is ready to run. Click **Run it now**.

5.12. After the crawler has finished, there are two tables in the **nycitytaxi** database: a table for the raw CSV data and a table for the transformed Parquet data.
...
6.1. On the **Services** menu, click **Athena**.

6.2. On the **Query Editor** tab, choose the database **nycitytaxi**.

![Athena query editor.png](images/Athena-query-editor.png)

6.3. Choose the **aws_glue_result_xxxx** table.

6.4. Type below standard SQL to query the data:
